...
export class FeishuShareSettingTab extends PluginSettingTab {
	...
	private addAppConfigSection(containerEl: HTMLElement): void {
		...
		// 创建标题
		const titleP = configDiv.createEl('p');
		titleP.createEl('strong', { text: '首次使用需要创建飞书应用：' });

		// 创建步骤列表
		const stepsList = configDiv.createEl('ol');
		...
		// 步骤2
		stepsList.createEl('li', { text: '创建企业自建应用或个人应用' });

		// 步骤3
		stepsList.createEl('li', { text: '在应用管理页面获取 App ID 和 App Secret' });

		// 步骤4
		const step4 = stepsList.createEl('li');
		step4.appendText('配置重定向 URL：');
		step4.createEl('code', { text: 'https://httpbin.org/get' });

		// 步骤5
		const step5 = stepsList.createEl('li');
		step5.appendText('申请以下权限：');
		const permList = step5.createEl('ul');
		permList.createEl('li', { text: 'contact:user.base:readonly (读取用户基本信息)' });
		permList.createEl('li', { text: 'docx:document (创建和管理文档)' });
		permList.createEl('li', { text: 'drive:drive (访问云文档)' });

		// 注意事项
		const noteP = configDiv.createEl('p');
		const noteEm = noteP.createEl('em', { text: '注意：由于技术限制，当前版本需要您手动配置应用信息。未来版本将简化此流程。' });
	}
	...
	private addAppCredentialsSection(containerEl: HTMLElement): void {
		...
		if (this.plugin.settings.appId && this.plugin.settings.appSecret) {
			const successSpan = statusDiv.createEl('span', { text: '✅ 应用配置已完成' });
			successSpan.style.color = 'var(--text-success)';
		} else {
			const errorSpan = statusDiv.createEl('span', { text: '⚠️ 请完成应用配置后再进行授权' });
	...
	private addProxySection(containerEl: HTMLElement): void {
		...
		const urlSpan = currentProxyDiv.createEl('span', { text: `当前代理地址: ${currentUrl}` });
	...
	private addUsageSection(containerEl: HTMLElement): void {
		...
		// 分享方式
		const shareP = usageDiv.createEl('p');
		shareP.createEl('strong', { text: '分享方式：' });

		const shareList = usageDiv.createEl('ul');
		shareList.createEl('li', { text: '在 Markdown 文件中，点击右上角菜单中的"分享到飞书"' });
		shareList.createEl('li', { text: '右键点击文件，选择"分享到飞书"' });
		shareList.createEl('li', { text: '使用命令面板搜索"分享到飞书"' });

		// 支持的内容
		const contentP = usageDiv.createEl('p');
		contentP.createEl('strong', { text: '支持的内容：' });

		const contentList = usageDiv.createEl('ul');
		contentList.createEl('li', { text: '✅ 标准 Markdown 格式（标题、列表、粗体、斜体等）' });
		contentList.createEl('li', { text: '✅ 网络图片链接（http/https）' });
		contentList.createEl('li', { text: '⚠️ Obsidian 特有语法会转换为普通文本' });
		contentList.createEl('li', { text: '❌ 本地图片需要手动上传到飞书' });
	}
	...
	private updateAuthStatus(element: HTMLElement): void {
		...
		if (this.plugin.settings.accessToken && this.plugin.settings.userInfo) {
			const statusSpan = element.createEl('span', { text: '✅ 已授权' });
			statusSpan.style.color = 'var(--text-success)';
			element.createEl('br');
			element.createEl('small', { text: `账户：${this.plugin.settings.userInfo.name}` });
		} else {
			const errorSpan = element.createEl('span', { text: '❌ 未授权' });
			errorSpan.style.color = 'var(--text-error)';
		}
	}
